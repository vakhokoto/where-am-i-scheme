...
;; helps to find intersection of the circle and circles right of that one
;; used in intersection-points

(define (helper-func elem seq)
  (if (null? seq) '()
    (append (intersect elem (car seq)) (helper-func elem (cdr seq)))))

;; takes the sequence and returns the list of all (i n) type lists where
;; i<=n and n is the length of the sequence

(define (permutate-seq seq)
  (if (< (length seq) 2) (cons seq '())
    ((lambda (x)
      (cons (cons (car seq) (car x)) x))(permutate-seq (cdr seq)))))

;; takes the list of circles format (r (Ox Oy))
;; andreturs all intersection points of all pairs

(define (intersection-points seq)
  (apply append (map (lambda (x y)
          (helper-func x (cdr y))) seq (permutate-seq seq))))

;; takes two points and return if they are the same or not

(define (same-points p1 p2)
  (and (= (car p1) (car p2)) (= (cadr p1) (cadr p2))))

;; takes the point A and list of points E and returns product of distances all pairs
;; (A B) where B is in E

(define (distance-product point seq)
  (apply * (map (lambda (new-point)
                  (if (same-points point new-point) 1
                    (dist point new-point))) seq)))

;; takes the list of points seq and returns pairs (P_A A)
...
(define (sort-points seq)
  (quicksort seq compare-distances))
...
(define (add-points p1 p2)
  (cons (+ (car p1) (car p2)) (cons (+ (cadr p1) (cadr p2)) '())))

;; finds the average point C for list seq
;; where Cx = sum of all Ax in seq / count of element in seq
...
;; takes the list of points and return list (rate-av-p; av-p)
...
(define (add-to-each elem seq)
  (if (null? seq) '()
    (cons (cons elem (car seq)) (add-to-each elem (cdr seq)))))

;; gives all permutation of seq
...
(define (concat seq1 seq2)
  (if (null? seq1) '()
    (cons (cons (car seq1) (list (car seq2))) (concat (cdr seq1) (cdr seq2)))))
...
(define (where-am-i radiuses centres)
  (quicksort (map (lambda (elem)
                  (best-estimate (concat radiuses elem))) (permutation centres)) compare-distances))
